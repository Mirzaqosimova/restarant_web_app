...
const data = await this.categoryRepository.findOneBy({name:payload.name })
if(data!= null){
    return res.json(ApiResponse.Conflict('Name already exists'))
}
await this.categoryRepository.save({name: payload.name,
file_name: payload.fileName})
.then((data)=>{return res.json(ApiResponse.Success(data))})
...
await this.categoryRepository.update({id:payload.id},{name: payload.name,
file_name: payload.fileName})
.then((data)=>{return res.json(ApiResponse.Success(data))})
.catch((err)=>(res.json(err)))
    }
...
if(data.products.length !== 0){
   return res.json(ApiResponse.Conflict('This category have products please delete first them'))
}
return this.categoryRepository.delete({id: payload.id})
.then((data)=>{return res.json(ApiResponse.Success(data))})
.catch((err)=>(res.json(err)))
    }
}